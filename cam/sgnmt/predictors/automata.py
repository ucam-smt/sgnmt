...
module with dependency on the PyFST library
...
import fst  # PyFST, NOT OpenFST's python interface
import glob
import logging
import os
import sys
...
class FstPredictor(Predictor):
    ...
    def predict_next(self):
        ...
        """
        if self.cur_node < 0 or not self.cur_fst[self.cur_node]:
            return {}
        scores = {arc.olabel: self.weight_factor*float(arc.weight)
                for arc in self.cur_fst[self.cur_node].arcs}
    ...
    def initialize(self, src_sentence):
        ...
        self.cur_node = self.cur_fst.start if self.cur_fst else -1
    ...
    def consume(self, word):
        ...
        if self.cur_node < 0:
            return
        from_state = self.cur_fst[self.cur_node]
        self.cur_node = -1
        for arc in from_state.arcs:
            if arc.olabel == word:
...
class NondeterministicFstPredictor(Predictor):
    ...
    def predict_next(self):
        ...
        for weight,node in self.cur_nodes:
            for arc in self.cur_fst[node].arcs:
...
class RtnPredictor(Predictor):
    ...
    def initialize(self, src_sentence):
        ...
        try:
            ...
            self.cur_fst = fst._fst.read(file_name) 
    ...
    def get_sub_fst(self, fst_id):
        ...
        sub_fst_path = "%s/%d/%d.fst" %  (self.root_path,
                                          ...
                                          fst_id)
        try:
            sub_fst = fst._fst.read(sub_fst_path)
