...
class Grammar(object):
  def __init__(self, lhs_to_rule_ids, rules):
    logging.info('Defining grammar')
    self.rule_id_to_rhs = dict() # reversed rhs
    self.lhs_to_rule_ids = lhs_to_rule_ids
    for rule_idx, rule in enumerate(rules):
      self.rule_id_to_rhs[rule_idx] = [int(r) for r in reversed(rule) if int(r) in lhs_to_rule_ids]
...
class ParsePredictor(Predictor):
    ...
    def __init__(self, grammar_path, nmt_predictor, word_out,
        ...
        self.cur_node = -1
        self.stack = []
        self.grammar = self.prepare_grammar()

    
    def prepare_grammar(self):
        lhs_to_rule_ids = collections.defaultdict(list)
        rules = []
        with open(self.grammar_path) as f:
            for idx, line in enumerate(f):
                nt, rule = line.split(':')
                nt = int(nt.strip())
                lhs_to_rule_ids[nt].append(idx)
                rules.append(rule.strip().split())      
        grammar = Grammar(lhs_to_rule_ids, rules)
        return grammar
    ...
    def predict_next(self, predicting_next_word=False):
        ...
        outgoing_rules = self.grammar.lhs_to_rule_ids[current_nt]
    ...
    def is_best_terminal(self, posterior):
        ...
        rhs = self.grammar.rule_id_to_rhs[best_rule_id]
    ...
    def find_word(self, posterior):
        ...
        if self.beam_size == 0:
          ...
          orig_states = [copy.deepcopy(self.get_state()) 
                                       for _ in range(self.beam_size)]
          next_words = utils.argmax_n(posterior, self.beam_size)
          while True:
            next_hypos = {}
            next_states = []
            for idx, word in enumerate(next_words):
              self.set_state(orig_states[idx])
              self.consume(word)
              new_post = self.predict_next(predicting_next_word=True)
              if self.is_best_terminal(new_post):
                return new_post
              next_states.append(self.get_state())
              for hypo, val in new_post.items():
                next_hypos[(idx, hypo)] = val
            best_post = sorted(next_hypos, 
                               key=lambda x: next_hypos[x],
                               reverse=True)[:self.beam_size]
            next_words = []
            for order, hypo in enumerate(best_post):
              idx, word = hypo
              next_words.append(word)
              orig_states[order] = next_states[idx]
    ...
    def initialize(self, src_sentence):
        ...
        self.stack = self.grammar.rule_id_to_rhs[utils.GO_ID]
    ...
    def consume(self, word):
        ...
        for arc in self.cur_fst.arcs(from_state):
            if arc.olabel == word:
                ...
                self.stack.extend(self.grammar.rule_id_to_rhs[arc.olabel])
