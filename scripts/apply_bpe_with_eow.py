...
- The default separator symbol is empty (instead of '@@')
- The </w> marker is *not* deleted in the output

This is the format used in SGNMT's T2T and GEC tutorials. SGNMT
expects this format (with </w> and without @@) when --preprocessing
or --postprocessing is set to 'bpe', and the original format (with
@@ and without </w>) when it is set to 'bpe@@'.
"""
...
def encode(orig, bpe_codes, bpe_codes_reverse, vocab, separator, version, cache, glossaries=None):
    ...
    # Following passage is commented out: do print </w>
